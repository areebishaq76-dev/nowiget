...
export default function Home() {
  ...
  const [messages, setMessages] = useState<{ question: string; answer: string; slug: string }[]>([]);
  ...
  const callAPI = async (question: string, familiarity: string, history: { question: string; answer: string }[]) => {
    const res = await fetch("/api/explain", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ confusion: question, familiarity, history: history.slice(-3) }),
    });
    return res;
  };
  ...
  const handleFamiliaritySelect = async (level: string) => {
    ...
    try {
      const res = await callAPI(confusion.trim(), level, []);
      const data = await res.json();

      if (!res.ok) {
        setError(data.error || "Something went wrong. Please try again.");
      } else {
        const newMessage = { question: confusion.trim(), answer: data.explanation, slug: data.slug || "" };
  ...
  const handleFollowUp = async () => {
    ...
    try {
      const res = await callAPI(question, familiarityLevel, conversationHistory);
      const data = await res.json();

      if (!res.ok) {
        setError(data.error || "Something went wrong. Please try again.");
      } else {
        const newMessage = { question, answer: data.explanation, slug: data.slug || "" };
        setMessages((prev) => [...prev, newMessage]);
        setCurrentSlug(data.slug || "");
        setFeedbackGiven(false);
        setConversationHistory((prev) => [...prev, { question, answer: data.explanation }].slice(-5));
        setTimeout(() => {
    ...
    <div className="flex flex-col min-h-full">
      ...
      <section className="relative flex flex-col items-center text-center pt-36 pb-24 px-6 md:pt-48 md:pb-32 overflow-hidden">
        ...
        <div className="relative w-full max-w-[700px] mt-12">
          ...
          {/* Answer display — all messages stacked */}
          {messages.length > 0 && (
            <div ref={answerRef} className="mt-6 flex flex-col gap-4 scroll-mt-24">
              {messages.map((msg, idx) => (
                <div key={idx} className="bg-card-bg rounded-2xl shadow-xl shadow-foreground/[0.04] border border-border/40 p-6 md:p-8 text-left">
                  {idx > 0 && (
                    <div className="mb-4 px-4 py-2 rounded-xl bg-primary/5 border border-primary/10">
                      <p className="text-xs text-primary font-medium">Follow-up: {msg.question}</p>
                    </div>
                  )}
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-secondary/60">{idx === 0 ? "Here's your explanation" : "Follow-up answer"}</p>
                    </div>
                  </div>
                  <div className="text-foreground leading-relaxed text-[15px] whitespace-pre-line">
                    {msg.answer}
                  </div>
                  {/* Feedback + Share only on last message */}
                  {idx === messages.length - 1 && (
                    <>
                          ...
                          <>
                            ...
                            <button
                              onClick={async () => {
                                ...
                                await fetch("/api/feedback", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ slug: currentSlug, helpful: true }) });
                              }}
                            ...
                            <button
                              onClick={async () => {
                                setFeedbackGiven(true);
                                await fetch("/api/feedback", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ slug: currentSlug, helpful: false }) });
                              }}
                      ...
                      <div className="mt-4 pt-4 border-t border-border/30">
                        {/* "Now I Get It" share card */}
                        <div className="mb-5 p-4 rounded-xl bg-gradient-to-br from-primary/8 to-primary/4 border border-primary/15">
                          <p className="text-xs font-semibold text-primary mb-1">Now I get it moment</p>
                          <p className="text-sm text-foreground font-medium leading-snug mb-3">
                            &ldquo;{msg.question.length > 120 ? msg.question.slice(0, 120) + "…" : msg.question}&rdquo;
                          </p>
                          <button
                            onClick={() => {
                              const shareText = `Now I get it! 💡\n\n"${msg.question.slice(0, 120)}"\n\nFinally understood thanks to NowIGet → ${msg.slug ? `https://nowiget.vercel.app/explain/${msg.slug}` : "https://nowiget.vercel.app"}\n\n#NowIGet`;
                          ...
                          >
                            {linkCopied ? "Copied! Paste anywhere →" : "Copy to share anywhere →"}
                          </button>
                        </div>
                        ...
                        <div className="flex items-center justify-between mb-3">
                          ...
                          {msg.slug && (
                            <a href={`/explain/${msg.slug}`} className="text-xs text-primary hover:underline">Permanent link →</a>
                          )}
                        </div>
                        <div className="flex items-center gap-2 flex-wrap">
                          <button
                            onClick={() => {
                              const text = `I finally understood: "${msg.question.slice(0, 100)}"`;
                              const url = msg.slug ? `${(process.env.NEXT_PUBLIC_SITE_URL || "https://nowiget.vercel.app")}/explain/${msg.slug}` : (process.env.NEXT_PUBLIC_SITE_URL || "https://nowiget.vercel.app");
                            ...
                            onClick={() => {
                              if (navigator.share) {
                                navigator.share({
                                  ...
                                  text: `"${msg.question.slice(0, 100)}"`,
                                  url: msg.slug ? `${(process.env.NEXT_PUBLIC_SITE_URL || "https://nowiget.vercel.app")}/explain/${msg.slug}` : (process.env.NEXT_PUBLIC_SITE_URL || "https://nowiget.vercel.app"),
                                });
                              }
                            }}
          ...
          {messages.length > 0 && !isLoading && (
            <div ref={followUpRef} className="mt-4 bg-card-bg rounded-2xl shadow-xl shadow-foreground/[0.04] border border-border/40 p-1.5">
              <textarea
                value={followUp}
              ...
              <div className="flex items-center justify-between px-3 pb-2 pt-1">
                ...
                <button
                  ...
                  className="w-full sm:w-auto px-6 h-[38px] rounded-xl bg-primary text-white font-semibold text-sm transition-all duration-200 hover:bg-primary-hover disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer"
                >
                  {isFollowUpLoading ? (
                    <>
